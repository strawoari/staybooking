...
public class ListingService {
    ...
    public void createListing(
        ...
        listingRepository.save(new ListingEntity(
                ...
                guestNumber,
                uploadedUrls,
                geometryFactory.createPoint(new Coordinate(37 , 42)) //new Coordinate(geoPoint.lat(), geoPoint.lon())
        ));
    }
    ...
    public void deleteListing(long hostId, long listingId) {
        ...
        if (listing.getHostId() != hostId) {
            throw new DeleteListingNotAllowedException("Host " + hostId +
                    " not allowed to delete listing " + listingId);
            //suppose host user who requested to do this is not the listing's owner
        }
        if (bookingService.existsActiveBookings(listingId)) {
            throw new DeleteListingNotAllowedException(
                    "Active bookings exist, not allowed to delete listing " + listingId);
        }
